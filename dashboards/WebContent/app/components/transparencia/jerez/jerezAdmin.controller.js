angular.module('jerezAdminController', [ 'dashboards', 'smart-table',
		'ngFileUpload', 'ui.bootstrap', ]);
angular
		.module('jerezAdminController')
		.controller(
				'adminCtrl',
				function($log, $scope, $rootScope, $routeParams, $http,
						$uibModal, $route) {
								...
								.then(
										function(response) {
											if (response.data.success) {
												...
												this.actividades_data = this.actividades_data_original.length > 0 ? this.actividades_data_original
														.slice(0)
														: [];
											}
											this.showloading = false;
					...
					this.select = function(index) {
						...
						$rootScope.fecha_inicio = moment(
								this.actividades_data[index].fecha_inicio,
								"DD/MM/YYYY HH:mm a").toDate();
						$rootScope.fecha_fin = moment(
								this.actividades_data[index].fecha_fin,
								"DD/MM/YYYY HH:mm a").toDate();
						...
						$rootScope.coord = "(" + $rootScope.coord_lat + ", "
...
angular
		...
		.controller(
				'editActivity',
				function($log, $scope, $rootScope, $http, $window,
						$uibModalInstance, $timeout, uiGmapGoogleMapApi,
						uiGmapIsReady, Upload, $timeout) {
					if ($rootScope.id <= 0) {
						$rootScope.id = -1;
						$rootScope.nombre = "";
							...
							.then(
									function(response) {
										if (response.data.success) {
											...
											$scope.documentos = $scope.original_documentos.length > 0 ? $scope.original_documentos
													.slice(0)
													: [];
										}
									}.bind(this),
					...
					$scope.deleteDoc = function(idDoc) {
						...
						$http
								.post('/STransparenciaDocumentos', data)
								.then(
										function(response) {
											if (response.data.success) {
												$http
														.post(
																'/STransparenciaDocumentos',
																{
																	action : 'getlist',
																	id : $rootScope.id,
																	t : (new Date())
					...
					$scope.save = function() {
						if ($rootScope.id > 0)
							...
							var data = {
								action : "create",
								nombre : $rootScope.nombre,
								descripcion : $rootScope.descripcion,
								fecha_inicio : tsFI,
								fecha_fin : tsFF,
								entidades : $rootScope.entidades,
								porcentaje_ejecucion : $rootScope.porcentaje_ejecucion,
								coord_lat : $rootScope.coord_lat,
								coord_long : $rootScope.coord_long,
								responsable_nombre : $rootScope.responsable_nombre,
								responsable_correo : $rootScope.responsable_correo,
								responsable_telefono : $rootScope.responsable_telefono,
								programa : $rootScope.programa,
								subprograma : $rootScope.subprograma
							};
							$http.post('/SSaveActividad', data).then(
									function(response) {
										if (response.data.success) {
											$rootScope.render = false;
					...
					$scope.update = function() {
						...
						var data = {
							...
							responsable_correo : $rootScope.responsable_correo,
							responsable_telefono : $rootScope.responsable_telefono,
							responsable_id : $rootScope.responsable_id
						};
						$http.post('/SSaveActividad', data).then(
								function(response) {
									if (response.data.success) {
...
angular.module('jerezAdminController').controller(
		'editCompras',
		function($log, $scope, $rootScope, $http, $window, $uibModalInstance,
				$timeout, uiGmapGoogleMapApi, uiGmapIsReady, Upload, $timeout) {
			this
