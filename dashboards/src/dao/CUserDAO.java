...
public class CUserDAO {
	...
	public static boolean createUser(String username, String plainPassword, String firstname, String secondname, String lastname,
			String secondlastname, String dependence, String position){
		try{
			...
			CUser user = new CUser(username, new Sha256Hash(plainPassword, salt, 1024).toBase64(), firstname, secondname, lastname, secondlastname, 
					dependence, position, salt.toBase64());
	        if(CDatabase.connect()){
	        	PreparedStatement pstm = CDatabase.getConnection().prepareStatement("INSERT INTO user(username, password, firstname, secondname, lastname, secondlastname, dependence, position, salt) "
	        			+ " VALUES(?,?,?,?,?,?,?,?,?) ");
	...
	public static boolean hasUserPermiso(String username, String permission){
		...
		try{
			if(CDatabase.connect()){
				PreparedStatement pstm = CDatabase.getConnection().prepareStatement("select count(*) from user_permiso where username = ?");
				pstm.setString(1, username);
				ResultSet rs = pstm.executeQuery();
				if(rs.next())
					ret = rs.getInt(1) > 0;
